...
SQL;
        ...
        public function Advs($param){
            if(isset($param['domaintype']) && $param['domaintype'] == 'site'){
                $list = D('Addons://Advs/Advs')->AdvsListSite($param);
            }else{
        	    $list = D('Addons://Advs/Advs')->AdvsList($param);
            }

            $this->assign('show_page',$param['show_page']);
//            echo $param['show_page'];
        	if(!$list) return ;
		    $this->assign('list',$list);
		if(is_array($param) && isset($param["tpl"])){
		    $this->display( $param["tpl"] );
		}else{
		    $this->display('widget');
		}
