...
class LinksModel extends Model{
	...
	protected function _after_find(&$result,$options) {
		$result['typetext'] =  $result['type'] == 1 ? '图片连接' : '文字连接';
		$result['statustext'] =  $result['status'] == 0 ? '禁用' : '正常';
		$result['create_time'] = date('Y-m-d', $result['create_time']);
		$position = $result['position'];
		$result['position_key'] = $position;
		if($position==0){
			$result['position'] = '频道全部';
		}else if($position==1){
			$result['position'] = '频道首页友情链接';
		}else if($position==2){
			$result['position'] = '频道百科页面';
		}else if($position==3){
			$result['position'] = '频道问答页面';
		}else if($position==4){
			$result['position'] = '频道首页推荐阅读';
		}else if($position==5){
			$result['position'] = '频道首页热门搜索';
		}else if($position==6){
			$result['position'] = '频道首页合作伙伴';
		}else if($position==7){
			$result['position'] = '频道品牌页面';
		}else if($position==8){
			$result['position'] = '频道排行榜页面';
		}else if($position==9){
			$result['position'] = '底部推荐链接';
		}
	}
