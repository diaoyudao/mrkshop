...
function get_list_field($data, $grid){
    ...
    if(!empty($grid['href'])){
		...
		$class="";
                if(strpos($href,'[DELETE]') !== false){
		    $class=" class='ajax-get confirm' ";
		}
		// 替换系统特殊字符串
                $href   =   str_replace(
                    array('[DELETE]','[EDIT]','[LIST]','[DISPLAYGOOD]','[DISPLAYARTICLE]','[COMMENT]'),
		    ...
		    'Web/Goods/detail/id/[id]',
		    'Web/help/detail/id/[id]',
		    'comment?id=[id]',
		    ),
                    $href);
		...
		if( $show=="浏览"){
		    $blank="target='_blank'";
                ...
                $val[]	= '<a '.$class.$blank.' href="'.U(trim($href)).'">'.$show.'</a>';
            }
        }
        $value  =   implode(' ',$val);
    }
...
function get_field_bymodel($model=null,$value = null, $condition = 'id', $field = null){
...
function clearcache( $ids="" ,$namestr="" ){ 
    if( $namestr ){
	if( is_array($ids) ){
	    foreach($ids as $k=>$v){
		$info = S($namestr.$v);
...
/***************************************************************
  *created date: 2015/6/13
  *created author:sheshanhu
  *content:采集评论时创建用户，如果
  *modefiy person:
  *modefiy date:
  *note:
****************************************************************/
function default_create_user($username){
    $uid = 0; 
    if(!empty($username)){
    //根据用户名查询用户是否存在，存在则返回UID
	...
	if(0 < $uid){ //注册成功
	    ...
	    $map['istestuser'] = 1;
	    $map['status'] = 1;
	    $userinfo = M('ucenter_member')->where($map)->order('ORDER BY RAND()')->limit(1)->select();
	    if(!empty($userinfo)){
	       $uid = $userinfo[0]['id'];
	    }
	}
...
function backU($url='',$vars='',$suffix=true,$domain=false) { 
    $url = R('Home/Index/admingeturl',array($url,$vars,$suffix,$domain) );
    return $url;
}