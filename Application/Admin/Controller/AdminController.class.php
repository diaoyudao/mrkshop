...
//                $this->error('未授权访问2!');
        ...
        $this->assign('status',array(1 => '启用',0 => '停用'));
        //设置图片路径
    ...
    public function del(){
        ...
        $name=CONTROLLER_NAME;
        $model = D ( $name ); 
        $list=$model->where ( $map )->setField ( 'status', - 1 );
        if ($list!==false) { 
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        $menus  = array();  session('ADMIN_MENU_LIST'.$controller);
        if(empty($menus)){
            // 获取主菜单
            $where['pid']   =   0;
            $where['hide']  =   0;
            ...
            foreach ($menus['main'] as $key => $item) {
                ...
                //wangcheng 百科和词条顶部都是百科选中
                if("baike/index" == strtolower($item['url']) && in_array(strtolower(CONTROLLER_NAME) ,array("entries","baike")) ){
                    $menus['main'][$key]['class']='current';
                }
            } 
            // 查找当前子菜单
...
//        $this->assign('show_productsgroup', IS_ROOT || $this->checkRule('Admin/ProductsGroup/index'));
    ...
    /**
     * ajax获取所有频道 
     * @param integer $domainid 频道id
     * @return json
     * @author wangcheng 
     */
    public function getbrandbydomain( $domainid=null ){
         //获取品牌
        if($domainid!=null){
            $map["domainid"]=intval($domainid);
        }
        $map["status"]=1;
        $list=M('SubdomainBrand')->where($map)->field("id,domainid,name")->select(); 
        $this->success($list);
    }
    /**
     * ajax获取指定类型分类
     ...
     * @author wangcheng 
     */
    public function getcategorybydomainandtype( $domainid=null ,$ismenu=null){
         //获取品牌
        if($domainid!=null){
            $map["domainid"]=intval($domainid); 
        }
        if($ismenu!=null){
            $map["ismenu"]=intval($ismenu);
            
        }
        $map["status"]=1; 
        $list=M('category')->where($map)->field("id,domainid,name,title")->select(); 
        $this->success($list);
    }
}
