...
//        $channel=   M('Subdomain')->where(array('status'=>1 ))->field('id,name')->select();
        $channel=   M('Subdomain')->where(array('status'=>1,'id'=>array('elt',1)))->field('id,name')->select();
...
//dump($cate);
        ...
        foreach ($cate as $key=>&$value){
            ...
            foreach($channel as $ck=> &$cv){ 
                ...
                if(!isset($cv['url'])){
                    $cv['url']   =   'Article/index?domainid='.$cv['id'];
                }
	    }
        } 
        $this->assign('nodes',     $channel);//$cate);
    ...
    public function index($cate_id = null, $model_id = null, $position = null,$group_id=null){
        ...
        if(!empty($cate_id)){
            ...
            if(is_null($model_id) && !is_numeric($models)){
                // 绑定多个模型 取基础模型的列表定义
                $model = M('Model')->getByName('article');
            }else{
                $model_id   =   $model_id ? : $models;
                //获取模型信息
                $model = M('Model')->getById($model_id);
                if (empty($model['list_grid'])) {
                    $model['list_grid'] = M('Model')->getFieldByName('article','list_grid');
                }                
            }
    ...
    public function add(){
        ...
        $categoryinfo =   M('Category')->find($cate_id);
        $info["domainid"] = $categoryinfo['domainid'];
	...
	$map["status"] = 1;
        ...
        //查询频道下的所有品牌
        $channelbrandlist=M('SubdomainBrand')->where("status >=1 and domainid=".$categoryinfo["domainid"])->getField("id,name"); 
    ...
    public function edit(){ 
        ...
        $categoryinfo =   M('Category')->find($data['category_id']);
    ...
    //2015/5/5 14:29 sheshanhu 品牌根据选择框选项值决定
	//$_POST["brandid"]= $info["brandid"];
        $res = $document->update();
    ...
    public function draftBox(){
        ...
        $Document   =   D('Document');
        $map        =   array('status'=>3,'uid'=>UID,'model_id'=>2);
