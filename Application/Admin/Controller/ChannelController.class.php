...
class ChannelController extends AdminController {
    ...
    public function index(){
        ...
        $list = M('Channel')->where($map)->order('domainid asc, sort asc,id asc')->select();
        $this->assign('list', $list);
        //获取频道
        
//        $channellist=M('Subdomain')->where("status >=1")->getField("id,name",true); 
        foreach ($this->channellist as $value) {
            $channellist[$value['id']] = $value['name'];
        }
        $this->assign("channellist",$channellist);
        $typeslist=array(0=>"频道-首页-顶部",1=>"文章-首页-顶部",2=>"频道-底部");
