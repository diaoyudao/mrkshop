    ...
    public function complete($id = 0) {
        if (IS_POST) {
            ...
            if ($_POST["id"]) {
                ...
                $Form->status =$status;
                $Form->backinfo =I('backinfo'); 
                $Form->complete_time = time(); //2015/5/8 14:14 sheshanhu订单完成状态修改时间
                $result = $Form->where("id='$id'")->save();
...
//获取购物清单数据表产品id，字段id
                ...
                if ($result) {
                    ...
                    $orderapi->addOrderLog($id, array('msg' => get_username($uid) . '更改了订单的状态为已完成', 'uid' => $uid, 'status' => $status, 'roleid' => $uid));
                    $this->success('更新成功', Cookie('__forward__'));
                } else {
                    $this->error('更新失败' . $id);
                }
