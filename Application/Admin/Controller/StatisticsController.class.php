...
//        dump($order);
         ...
         * 当天退货金额、种类、数量计算
         */
        $data = M("order_refund")->where(array('refund_state' => '0', 'refund_type' => 2))->select();
        $cqtime = NOW_TIME;
        foreach ($data as $k => $val) {
            ...
            if ($c == $d) {//当天
                ...
                $goodid = $val['goods_id'];
                /* 退货中的商品总额 */
                $back+= $val['refund_amount'];
                /* 退货中的商品件数 */
                $backnum+=$val['goods_num'];
                /* 退货中的商品种类数 */
                $backcount+=1;
            }
        }
         ...
         * 当天退款金额、种类、数量计算
         */
        $data = M("order_refund")->where(array('refund_state' => '0', 'refund_type' => 1))->select();
        $eqtime = NOW_TIME;
        foreach ($data as $k => $val) {
         ...
         * 当天申请取消订单金额、种类、数量计算
         */
        $cdata = M("order")->where(array('status' => '-2'))->select();
        $cqtime = NOW_TIME;
        foreach ($cdata as $k => $val) {
            /* 格式时间戳为 20141024 */
            $q = date('Ymd', $cqtime);
            $time = $val['cancel_time'];
            $s = date('Ymd', $time);
            if ($q == $s) {
                //当天
                /* 取消订单的商品总额 */
                $cancel+= $val['pricetotal'];
                /* 取消订单的商品件数 */
                $cancelnum+=$val['goods_sum'];
                /* 取消订单的商品种类数 */
                $cancelcount+=1;
            }
        }
...
//        $data = M("change")->where("status='1'")->select();
         ...
         * 本周申请取消订单金额、种类、数量计算
         */
        $cdata = M("order")->where("status='-2'")->select();
        foreach ($cdata as $k => $val) {
            /* 格式时间戳为 20141024 */
            $cantime = $val['cancel_time'];
        ...
        $week = "本周开始时间{$stime}结束时间{$etime}";
        $this->assign('dtime', $week);
        $this->assign('title', $title);
        $this->display('index');
    }
