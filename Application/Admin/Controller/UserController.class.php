...
class UserController extends AdminController {
    ...
    public function index(){
        $nickname       =   I('nickname');
        $map['ucenter_admin.status']  =   array('egt',0);
        ...
        $MemberViewModel = D("MemberAdminView");
    ...
    public function add($username = '', $password = '', $repassword = '', $email = '',$mobile=''){
        if(IS_POST){
            ...
            $is_admin = 1;  // 添加管理员
            $User   =   new UserApi;
            $uid    =   $User->register($username, $password, $email,$mobile,$is_admin);
            if(0 < $uid){ //注册成功
                $user = array('uid' => $uid, 'nickname' => $username, 'status' => 1,'is_admin'=>$is_admin);
                if(!M('Member')->add($user)){
                    ...
                    // ucenter_admin 添加一条记录
                    M('Ucenter_admin')->add(array('member_id'=>$uid,'status'=>1));
    ...
    private function showRegError($code = 0){
        switch ($code) {
            ...
            case -12: $error = '用户名必须填写！'; break;
            case -13: $error = '手机号必须填写！'; break;
            default:  $error = '未知错误';
        }
        return $error;
    }
