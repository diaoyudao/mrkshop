...
class CategoryModel extends Model{
    protected $_validate = array(
        array('domainid', 'require', '频道不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH),
        array('name', 'require', '标识不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH), 
        array('name', 'checkName', '标识已经存在', self::VALUE_VALIDATE, 'callback', self::MODEL_BOTH),
        array('title', 'require', '名称不能为空', self::MUST_VALIDATE , 'regex', self::MODEL_BOTH), 
        array('title', 'checkTitle', '该分类名称已经存在', self::VALUE_VALIDATE , 'callback', self::MODEL_BOTH),
     ...
     * @author 麦当苗儿 <zuojiazi@vip.qq.com>
     */
    public function getTree($id = 0, $field = true,$where=array()){
        /* 获取当前分类信息 */
        if($id){
            $info = $this->info($id);
            $id   = $info['id'];
    ...
    protected function checkName(){
        ...
        if ($category['pid'] == 0) {
            ...
            $map['pid'] = array('in', $this->getChildrenId($root['id']));
        }
    ...
    protected function checkTitle(){
        $title = I('post.title'); 
        $id = I('post.id', 0); 
        $map = array('title' => $title, 'id' => array('neq', $id),'ismenu'=>$ismenue, 'status' => array('neq', -1)); 
        $res = $this->where($map)->getField('id');
        if ($res) {
            return false;
        }
