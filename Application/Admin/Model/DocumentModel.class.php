...
class DocumentModel extends Model{
    ...
    protected $_validate = array(
        ...
        array('title', 'checkTitle', '该标题已经存在', self::VALUE_VALIDATE , 'callback', self::MODEL_BOTH),
    ...
    public function update($data = null){
        ...
        //如果是公告，则没有封面图片也可以上传成功
        $cover_id = I('cover_id','');
        if(empty($cover_id) && I('category_name')=="notice" ){
            $_POST['cover_id'] = 0;
        }
        /* 获取数据对象 */
    ...
    protected function saveAttribute( $gid=0 ){
        //判断是否是商品
        if( I("model_id")==5 ){
            //设置扩展属性
            $this->doattr($gid);
        ...
        //$this->setrecommend($gid);
    }
    
    function movepic($gid){
        $domainid=$_POST["domainid"];
        $getdatabyurl = I("getdatabyurl");
        if( $getdatabyurl!="" ){//判断如果是通过采集的，则处理封面图，图集，及详细内容里面的图片转移，属性已在上一步操作完成
            $cover_path = I("local_cover_id");//转移封面图
            if($cover_path && file_exists($cover_path) ){
                $update=array();
                $data=array();
                //判断是否已经存在,如果存在则返回ID,没有则转移图片并插入到picture表
                $data['md5']=md5_file( $cover_path );
                $data['sha1']= sha1_file( $cover_path );
                $info =  M("picture")->where($data)->find();
                if($info){
                    $update["cover_id"]=$info["id"];
                }else{
                    $data["types"]=1;
                    $data["status"]=1;
                    $data["create_time"]=time();
                    $data["path"]=str_replace("./Uploads/Caiji/Picture/".$domainid."/","",$cover_path);
                    $update["cover_id"]=M("picture")->add($data);
                    //物理转移图片
                    $p=pathinfo("./Uploads/Picture/".$domainid."/".$data["path"]);
                    $this->mkdirp( $p["dirname"] );
                    copy($cover_path,"./Uploads/Picture/".$domainid."/".$data["path"]);
                }
                $update["id"]=$gid;
                M("document")->save($update);
            }
            $pic_path=I("local_pics");
            if( $pic_path ){
                $update=array();
                $newpic=array();
                $pics=I("pics");
                ...
                foreach($pic_path as $k=>$v){
                    ...
                    if( file_exists($v) ){
                        ...
                        if($info){
                            ...
                            $data["status"]=1;
                            $data["create_time"]=time();
                            $data["path"]=str_replace("./Uploads/Caiji/Picture/".$domainid."/","",$v);
                            $pic[]=M("picture")->add($data);
                            $p=pathinfo("./Uploads/Picture/".$domainid."/".$data["path"]);
                            $this->mkdirp( $p["dirname"] );
                            copy($v,"./Uploads/Picture/".$domainid."/".$data["path"]);
                        }
                    }
                }
                if($pic){
                    ...
                    $newpic=array_unique($newpic);
                    if($newpic){
                        $update["pics"]=implode(",",$newpic);
                    }
                }
                if($update){
                    $_POST["pics"]=$update["pics"];
                }
            }
            $content = I("content");
            if( $content ){
		$content=str_replace("/Uploads/Caiji/","./Uploads/Caiji/",$content);
                preg_match_all('/src="(.*?)"/',$content,$contentpics);
                $imgarray = $contentpics[1];
                if($imgarray){
                    foreach($imgarray as $k=> $v){
                        if( file_exists( $v ) ){
                            $newpath=str_replace("/Caiji/Editor/","/Editor/",$v);
                            $p=pathinfo($newpath);
                            $this->mkdirp( $p["dirname"] );
                            copy($v,$newpath);
                        }
                    }
                    $content=str_replace("/Caiji/Editor/","/Editor/",$content);
                    $_POST["content"]=str_replace("./Uploads/","/Uploads/",$content);
                }
            }
            //保存至已采集库
            $caijidata=array();
            $caijidata["url"]=$getdatabyurl;
            $caijidata["goodid"]=$gid;
            $caijidata["create_time"]=time();
            if(strpos($getdatabyurl,'taobao.com') !== false){
                $caijidata["urlfrom"]="taobao";
            }else if(strpos($getdatabyurl,'tmall.com') !== false){
                $caijidata["urlfrom"]="tmall";
            }else{
                $caijidata["urlfrom"]="未知";
            }
            M("caiji")->add($caijidata);
        }
    }
    
    //设置推荐位置关联表（商品和文章共用）
    function setrecommend( $gid ){
        $data=array();
        $map["domainid"] = $_POST["domainid"];
        $map["recommendid"] = $gid;
        $position= I("position");
        $re = M("recommend")->where($map)->delete();
        if( $position ){
            $arr=array();
            $data=array();
            $data["domainid"] = $_POST["domainid"];
            $type=0;
            if( I("model_id")==5 ){
                $type=1;
            }
            $data["types"] = $type;
            $data["recommendid"] = $gid;
    ...
    function doattr( $id ){
        ...
        {
            // 取得原有的属性值
            $arr = M("GoodAttr")->where("gid=".$id)->field("id")->select();
            $date_delete=array();
            foreach( $arr as $k=>$v ){
...
//            dump($_POST['inputtypes']);
//            dump($_POST['attr_types_list']);
//            dump($_POST['attr_id_list']);
//            dump($_POST['attr_value_list']);exit;
            // 循环现有的，根据原有的做相应处理
            if(isset($_POST['attr_id_list']))
            {
                foreach ($_POST['attr_id_list'] AS $key => $attr_id)
                {
                    ...
                    $attr_price = floatval($attr_price);
                    $gattrid = $_POST['attr_goodattr_id_list'][$key];
                    $attr_types = $_POST['attr_types_list'][$key];
                    $inputtypes = $_POST['inputtypes'][$key];
                    $keys = isset($_POST['attr_keys_list'][$key]) ? $_POST['attr_keys_list'][$key]:"";
                    ...
                    if (!empty($attr_value) && $attr_value!="undefined" &&  $attr_value!="false")
                    {
                        ...
                        {
                            // 如果原来有，标记为更新
                            if( isset($date_delete[$gattrid]) ) unset($date_delete[$gattrid]);
                            $data=array();
                            $data["attributeid"]=$attr_id;
                            $data["keys"] = $keys;
                            $data["id"] = $gattrid;
                            if($attr_types==0){
                                $data["price"] = "";
                                $data["attributesubid"]=0;
                                $data["value"] = $attr_value;
                                if( $inputtypes==1 ){
                                    $data["keys"] = M("GoodAttribute")->where("id=".$attr_id)->getField("name");
                                    $data["value"] = M("GoodAttributeSub")->where("id=".$attr_value)->getField("name");
                                    $data["attributesubid"]=$attr_value;
                                }
                                else{
                                    $data["attributesubid"]=$attr_value;
                                    $data["value"] = $attr_value;
            ...
            if($date_delete){
                ...
                M("GoodAttr")->where($map)->delete();
            }
            if($data_insert) M("GoodAttr")->addAll($data_insert);
        }
        
        //处理优惠价格
        if ((isset($_POST['volume_number']) && isset($_POST['volume_price'])))
        {
            M("GoodVolumePrice")->where("gid=".$id)->delete();
            $data_insert=array();
            foreach ($_POST['volume_number'] as $key => $num) {
                $price = $_POST['volume_price'][$key];
                if(intval($num)>0 && floatval($price) >0){
                    $data=array();
                    $data["gid"] = $id;
                    $data["pricetype"] = 1;
                    $data["price"] = $price;
            ...
            if($data_insert) M("GoodVolumePrice")->addAll($data_insert);
        }
    }
    /**
     * 获取数据状态
    ...
    public function checkDocumentType($type = null, $pid = null){
        ...
        $ptype = is_numeric($pid) ? $category->getFieldById($pid, 'type') : $category->getFieldByName($pid, 'type');
