...
class CommentController extends HomeController {
    ...
    public function evaluation() {
        if (is_login()) {
            ...
            $aboutproduct = $goodsapi->getDocument(array(), 'sales DESC', 4);
...
//            dump($goodsinfo);
    ...
    public function detail() {
        $goodsid = I('goodid');
        $orderid = I('orderid');
        $comment = M("comment")->where(array('goodid' => $goodsid, 'orderid' => $orderid))->find(); // 'uid' => is_login()
        if ($comment['pics']) {
...
//                        }
        ...
        $comment['icon'] = M('document')->where(array('id' => $goodsid))->getfield('cover_id');
        $this->assign('info', $comment);
        //热卖推荐
        $goodsapi = new \Api\Api\GoodsApi();
        $aboutproduct = $goodsapi->getDocument(array(), 'sales DESC', 4);
        $this->assign('aboutproduct', $aboutproduct);
        $this->meta_title = "评价详情";
        $this->display();
    }
