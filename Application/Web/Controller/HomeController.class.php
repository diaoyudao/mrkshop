...
class HomeController extends CoreController {
    ...
    protected function _initialize() {
        ...
        if(isMobil()  == '手机登录'){
            header("Location:".WAP_SITE_URL.$_SERVER['REQUEST_URI']);
        }
        //设置图片路径
...
//        dump($helpcatelist);
    ...
    public function sendPhoneMsg($type = 1, $tel = "") {
        ...
        $cCount = M("phone_verify")->where($cmap)->count();
        if ($cCount > 0) {
            return false;
        }
        $code = "";
...
//         $appSecret = C('message.AppSecret');
        ...
        $map["type"] = $type;
        $map["phone"] = $tel;
        $result = M("phone_verify")->where($map)->delete();
        $data = array();
        ...
        $data["status"] = 1;
        $result = M("phone_verify")->add($data);
        if ($result) {
            return $data['code'];
        } else {
            return false;
