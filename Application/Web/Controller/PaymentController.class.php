...
//             $sysconfig = api('Config/lists');
            ...
            $orderinfo = $order->where(array('orderid' => $id))->find(); // 未支付
            if (empty($orderinfo)) {
                $this->error('订单信息不存在，或参数错误');
            }
            if ($orderinfo['status'] != -1) {
                $this->success('订单已付款，请查看订单物流信息', U('Order/index'));
               // $this->success('订单已付款，请查看订单物流信息');
            }
...
//         $sysconfig = api('Config/lists');
        ...
        $payid = I('payid');
        $paycode = I('paycode');
        $paymentinfo = M('payment')->where(array('paycode' => $paycode))->find();  //'id' => $payid, 
        ...
        if ($order_total == 0) {
            $this->error('订单金额为0，不需支付，请联系客服' . C('CONTACT'));
        }

        if ($orderid) {
            //查询订单 商品标题、描述
            $goodid_items = implode(',', M('shoplist')->where("tag='$orderid'")->getField("goodid", true));
            $map = array();
            $map['id'] = array('in', $goodid_items);
            $_items = M('document')->where($map)->getField("title,description", true);
            if ($_items) {
                $title = '';
                $description = '';
        ...
        switch ($paycode) {
            case 'alipay':
                ...
                $vo->setBody($body)
                        ...
                        ->setCallback(U('Web/Payment/alipaysuccess'))/* 设置支付完成后的后续操作接口 */
                        ->setUrl(U("Web/Payment/over")) /* 设置支付完成后的跳转地址 */
...
//                 $this->error('其他支付请联系客服，请耐心等待！' . C('CONTACT'), Cookie('__forward__'));
                ...
                $wxpay->return_url =SITE_URL."Payment/wx_notify.html";// C('DOMAIN') . U("Web/Payment/wx_notify");
...
//        exit(1);
    ...
    public function check_order_status() {
        ...
        if ($status >= 1) {
            $this->success('支付成功', U('Order/details',array('orderid'=>$orderId)));
//            $this->redirect(U('Order/details',array('orderid'=>$orderId)));
        }
        $this->error('等待支付');
    }
...
//                     redirect($payinfo['url']);
    ...
    public function alipaysuccess($param) {
