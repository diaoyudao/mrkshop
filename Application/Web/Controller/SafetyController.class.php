...
class SafetyController extends HomeController {
    ...
    public function bind_mobile() {
        ...
        if (IS_AJAX) {
            $phone = I('mobile');
            $code = I('code');
//            if (I('get.do') == 'update_mobile') {
//                $code = I('get.code');
//                $phone = $ucenter_member['mobile'];
//            }
            $code_m["code"] = $code;
            $code_m["phone"] = $phone;
...
//                    'mobile_auth' => 1
                ...
                $re = M("phone_verify")->where(array('phone' => $phone))->delete();
    ...
    public function changeMobile() {
        if (IS_AJAX) {
            ...
            $old_data['code'] = $code;
            $counts = M("phone_verify")->where($old_data)->count("*");
            if ($counts == 0) {
...
//                    'mobile_auth' => 1
                ...
                $re = M('member')->where('uid=' . $uid)->setField(array('mobile_auth' => 1));
                $re = M("phone_verify")->where(array('phone' => $phone))->delete();
                $re = M("phone_verify")->where(array('phone' => $old_mobile))->delete();
                $this->success('绑定成功', U('Safety/safety'));
            }
        } else {
    ...
    function getMobileCode() {
        ...
        $type = I("post.type", 3); // 3： 绑定手机  2： 找回密码 1:注册  4：旧手机验证码
     ...
     * 提示信息
     */
    public function show_message($message,$jumpUrl){
