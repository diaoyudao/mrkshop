...
class ShopcartModel extends Model {
    ...
    public function getCntByuid($map) {
        $user = D("member");
        $uid = $user->uid();
        if (!$map['uid']) {
            $map["uid"] = $uid;
    ...
    public function getTotalbysort($sort) {
        $user = D("member");
        $uid = $user->uid();
        if (!$map['uid']) {
            $map["uid"] = $uid;
    ...
    public function getPriceByuid($map) {
        $uid = D("member")->uid();
        if (!$map['uid']) {
            $map["uid"] = $uid;
        }
    ...
    public function getNumByuid($map) {
        $user = D("member");
        $uid = $user->uid();
        if (!$map['uid']) {
            $map["uid"] = $uid;
    ...
    public function inc($sort, $inputnum = 0) {
        $user = D("member");
        $uid = $user->uid();
        $cart = D("shopcart");
     ...
     * @return type
     */
    public function dec($sort, $number = 1) {
        $user = D("member");
        $uid = $user->uid();
        $cart = D("shopcart");
        $num = $cart->where("sort='$sort'and uid='$uid'")->getField("num");
