...
<body>
    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal">
        ...
        <div class="form-item" style="display: none;">
            ...
            <div class="controls">
                <select class="select" id="domainid" name="domainid"> 
                    <?php if(is_array($distributionlist)): $i = 0; $__LIST__ = $distributionlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$distributionvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($distributionvo["id"]); ?>" <?php if(($distributionvo[id]) == $info['domainid']): ?>selected<?php endif; ?>><?php echo ($distributionvo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">配送标题<span class="check-tips">（用于显示的文字）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="title" value="<?php echo ((isset($info["title"]) && ($info["title"] !== ""))?($info["title"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">配送金额<span class="check-tips"></span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="price" value="<?php echo ((isset($info["price"]) && ($info["price"] !== ""))?($info["price"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">配送描述<span class="check-tips"></span></label>
            <div class="controls">
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-small" name="weight" value="<?php echo ((isset($info["weight"]) && ($info["weight"] !== ""))?($info["weight"]):'0'); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">配送方式类型<span class="check-tips">（配送方式类型 0：线上配送;1:门店配送）</span></label>
            <div class="controls">
                <select name='type'>
                    <option value="">配送类型</option>
                    <option value='0' <?php if(($info["type"]) == "0"): ?>selected="selected"<?php endif; ?> >线上配送</option>
                    <option value='1' <?php if(($info["type"]) == "1"): ?>selected="selected"<?php endif; ?> >门店配送</option>
                </select>
                <!--<input type="text" class="text input-small" name="type" value="<?php echo ((isset($info["type"]) && ($info["type"] !== ""))?($info["type"]):'0'); ?>">-->
            </div>
        </div>
        <div class="form-item">
