	...
	<div class="tab-content">
	    ...
	    <?php $_result=parse_config_attr($model['field_group']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><div id="tab<?php echo ($key); ?>" class="tab-pane <?php if(($key) == "1"): ?>in<?php endif; ?> tab<?php echo ($key); ?>">
				...
				<select name="domainid" class="select required" onchange="getcategory (this.options[this.options.selectedIndex].value,'ke_child','请选择')">
				    ...
				    <?php if(is_array($domainlist)): foreach($domainlist as $zhong_key=>$c_zhong): ?><option value="<?php echo ($c_zhong['id']); ?>" <?php if(($info[category_id] == $c_zhong['id']) || ($ke_id == $c_zhong['id'])): ?>selected<?php endif; ?> ><?php echo ($c_zhong['name']); ?></option><?php endforeach; endif; ?>
				</select>
		    ...
		    <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 2): ?><div class="form-item cf">
				...
				<?php switch($field["type"]): case "pictures": ?><!-- 多图上传 --> 
					...
					<?php if(!empty($valArr)): if(is_array($valArr)): $i = 0; $__LIST__ = $valArr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="upload-pre-item">
						    <img src="/mrkshop/Uploads/Picture/<?php echo ($v["domainid"]); ?>/<?php echo (get_cover($v,'path')); ?>" data-id="<?php echo ($v); ?>"/>
					...
					$(function(){
					    /* 初始化上传插件*/
//					    var domainid= '<?php echo ($info["domainid"]); ?>';
					    ...
					    $(document).delegate('.btn-close-<?php echo ($field["name"]); ?>', 'click', function(event) { 
				...
				<?php case "string": ?><input type="text" class="text input-large" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>">
				    ...
				    <a class="market_price_setted" href="javascript:;" style="cursor: pointer;">点击按市场价计算</a>
                                    ...
                                    <div id='member_level_price_box'></div><?php endif; break;?>
				<?php case "textarea": ?><label class="textarea input-large">
				    <textarea name="<?php echo ($field["name"]); ?>"><?php echo ($field["value"]); ?></textarea>
				</label><?php break;?>
				<?php case "date": ?><input type="text" name="<?php echo ($field["name"]); ?>" class="text date" value="{time()|date='Y-m-d',###}" placeholder="请选择日期" /><?php break;?>
				<?php case "datetime": ?><input type="text" name="<?php echo ($field["name"]); ?>" class="text time" value="{time()|date='Y-m-d',###}" placeholder="请选择时间" /><?php break;?>
				...
				<?php case "select": ?><select name="<?php echo ($field["name"]); ?>">
					<?php if($field['name'] == 'product_type'): if(is_array($goodtypelist)): $i = 0; $__LIST__ = $goodtypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($field["value"]) == $key): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                            
                                        <?php elseif($field['name'] == 'warehouse'): ?>
                                            <?php if(is_array($distribution)): $i = 0; $__LIST__ = $distribution;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($field["value"]) == "vo.id"): ?>selected<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					...
					<?php else: ?>
					    <?php $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($field["value"]) == $key): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
				    </select><?php break;?>
...
//									'formData'      : {'domainid' : domainid},
			...
			<?php if($field['name'] == 'stock'): ?><!--仓储下面紧跟优惠价格-->
...
<!--				<div class="form-item cf">
			...
			<?php elseif($field['name'] == 'promote_price'): ?><!--仓储下面紧跟优惠价格-->
 ...
 $promote_start_time= $data['promote_start_time'] ? date('Y-m-d',$data['promote_start_time']):date('Y-m-d'); $promote_end_time= $data['promote_end_time'] ? date('Y-m-d',$data['promote_end_time']):date('Y-m-d'); ?>
	    ...
	    <div id="tab<?php echo (1+count(parse_config_attr($model['field_group'])));?>" class="zdysx tab-pane tab<?php echo (1+count(parse_config_attr($model['field_group'])));?>">
	    ...
	    <div class="form-item cf">
		    ...
		    <!--<input type="hidden" name="domainid" id="domainid" value="<?php echo ((isset($info["domainid"]) && ($info["domainid"] !== ""))?($info["domainid"]):''); ?>">-->
...
$(function(){
	...
	$.ajax({
	    type: 'POST', 
	    url: url , 
	    data:{domainid:'<?php echo ($info["domainid"]); ?>',keywords:sk},
	    success: function(json){
		var data=json.info;
		var html="";
		if( data ){
...
function getproductinfobyurl(){
	...
	var url="<?php echo U('GetData/getdatataobao');?>";
    }else if (caiji_domain=="tmall"){
	var url="<?php echo U('GetData/getdatatmall');?>";
    }else if (caiji_domain=="yao"){
	var url="<?php echo U('GetData/getdatayao');?>";
    }else if (caiji_domain=="1688"){
	var url="<?php echo U('GetData/getdataalibaba');?>";
    }
    else{
	...
	type: 'post', 
	url: url ,
	data:{url:caiji_url,domainid:'<?php echo ($info["domainid"]); ?>'},
	success: function(json){
	    if(json.status==1){
		...
		//标题及内容插入
		$.getScript('/mrkshop/Public/static/kindeditor/kindeditor-min.js', function() {
