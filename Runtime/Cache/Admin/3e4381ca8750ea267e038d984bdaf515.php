          ...
          <th>排序</th>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><img style="max-width:50px;max-height: 50px;" src="/mrkshop/Uploads/Picture/<?php echo ($channel['id']); ?>/<?php echo (get_cover($channel["icon"],'path')); ?>"/></td>
						<td><?php echo ($channel["name"]); ?></td>
						<td><?php echo ($channel["url"]); ?></td>
            <td><?php echo ($channel["bindgroup"]); ?></td>
            <td><?php echo ($channel["mark"]); ?></td>
            <td><?php echo ($channel["weight"]); ?></td>
						<td>
