	...
	<div class="data-table table-striped">
		<table>
          ...
          <th>排序</th>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$payment): $mod = ($i % 2 );++$i;?><tr>
						<td><input class="ids row-selected" type="checkbox" name="" id="" value="<?php echo ($payment['id']); ?>"> </td>
						<td><?php echo ($payment["id"]); ?></td>
						<td><a href="<?php echo U('edit?id='.$payment['id']);?>"><?php echo ($payment["payname"]); ?></a></td>
            <td><?php echo ($payment["paycode"]); ?></td>
						<td><?php echo ($payment["paydec"]); ?></td>
             <td><?php echo ($payment["weight"]); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$payment['id']);?>">编辑</a>
							<a href="<?php echo U('setStatus?ids='.$payment['id'].'&status='.abs(1-$payment['status']));?>" class="ajax-get"><?php echo (show_status_op($payment["status"])); ?></a>
							<a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$payment['id']);?>">删除</a>
						</td>
