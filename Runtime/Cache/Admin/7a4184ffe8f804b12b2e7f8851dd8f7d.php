...
<?php if(is_array($nodes)): $i = 0; $__LIST__ = $nodes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$submenu): $mod = ($i % 2 );++$i;?><h3>
	<i class="icon <?php if(($submenu["current"]) != "1"): ?>icon-fold<?php endif; ?>"></i>
	<a class="item" href="#"><?php echo ($submenu["name"]); ?></a>
    </h3>
    <ul class="side-sub-menu <?php if(($submenu["current"]) != "1"): ?>subnav-off<?php endif; ?>">
    <?php if(is_array($submenu[articlecat])): $i = 0; $__LIST__ = $submenu[articlecat];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub_menu): $mod = ($i % 2 );++$i;?><li>
        <!-- 子导航 -->
        <?php if(!empty($sub_menu)): ?><h3>
            	<i style="margin-left: 26px;" class="icon <?php if(($sub_menu['current']) != "1"): ?>icon-fold<?php endif; ?>"></i>
            ...
            <ul class="side-sub-menu <?php if(($sub_menu["current"]) != "1"): ?>subnav-off<?php endif; ?>">
                <?php if(is_array($sub_menu['_child'])): $i = 0; $__LIST__ = $sub_menu['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><li <?php if($menu['id'] == $cate_id or $menu['current'] == 1): ?>class="current"<?php endif; ?>>
                        <?php if(($menu['allow_publish']) > "0"): ?><a style="margin-left: 26px;" class="item" href="<?php echo (U($menu["url"])); ?>"><?php echo ($menu["title"]); ?></a><?php else: ?><a style="margin-left: 26px;" class="item" href="javascript:void(0);"><?php echo ($menu["title"]); ?></a><?php endif; ?>
        ...
        <!-- /子导航 --></li><?php endforeach; endif; else: echo "" ;endif; ?>
...
<script>
    ...
    <div class="data-table">
		<table>
			...
			<td><a target="_blank" title="浏览" href="<?php echo backU('Home/Article/detail',array('id'=>$data['id']));?>">浏览</a></td>
