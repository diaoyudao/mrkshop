...
<body>
	...
	<div class="main-title">
		<h2>用户列表
                    <a href="<?php echo U('Member/index');?>">全部</a>
                    <a href="<?php echo U('Member/index',array('member_type'=>0));?>">未绑定</a>
                <?php if(is_array(C("MEMBER_TYPE"))): foreach(C("MEMBER_TYPE") as $key=>$vo): if(($member_type) != $key): ?><a href="<?php echo U('Member/index',array('member_type'=>$key));?>"><?php echo ($vo); ?></a><?php else: echo ($vo); endif; ?>&nbsp;<?php endforeach; endif; ?>
                </h2>
	</div>
	<div class="cf">
		<div class="fl">
    ...
    <tbody>
      ...
      <td><?php echo ($vo["email"]); ?></td>
			...
			<td>
                                <?php if($vo["member_type"] == 1): ?>区域代理|<a href="<?php echo U('member/bank?uid='.$vo['uid']);?>" class="authorize">财务信息</a>
                                <?php elseif($vo["member_type"] == 2): ?>门店|<a href="<?php echo U('member/bank?uid='.$vo['uid']);?>" class="authorize">财务信息</a>
                                <?php else: ?> 终端消费者<?php endif; ?>
                        </td>
			...
			<td>
				...
				<a href="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>" class="ajax-get">启用</a><?php endif; ?>
                            <a href="<?php echo U('Member/edit?uid='.$vo['uid']);?>" >编辑</a>
                            <a href="<?php echo U('Member/changeStatus?method=deleteUser&id='.$vo['uid']);?>" class="confirm ajax-get">删除</a>
                <!--<?php if(($vo["member_type"]) != "3"): ?><a href="<?php echo U('member/bank?uid='.$vo['uid']);?>" class="authorize">财务信息</a><?php endif; ?>-->
                </td>
	...
	<script src="/mrkshop/Public/static/thinkbox/jquery.thinkbox.js"></script>
