...
<body>
    ...
    <div class="data-table table-striped">
        <table>
            ...
            <tbody>
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$distribution): $mod = ($i % 2 );++$i;?><tr>
                        <td><input class="ids row-selected" type="checkbox" name="" id="" value="<?php echo ($distribution['id']); ?>"> </td>
                        <td><?php echo ($distribution["id"]); ?></td>
                        <td><?php if(($distribution["type"]) == "1"): ?>门店配送<?php endif; ?> <a href="<?php echo U('edit?id='.$distribution['id']);?>"><?php echo ($distribution["title"]); ?></a></td>
                        <td><?php echo ($distribution["price"]); ?></td>
                        <td><?php echo ($distribution["description"]); ?></td>
                        <td><?php echo ($distribution["weight"]); ?></td>
                        <td>
                            <a title="设置配送区域" href="<?php echo U('shipping?id='.$distribution['id']);?>">设置配送区域</a>
                            <a title="编辑" href="<?php echo U('edit?id='.$distribution['id']);?>">编辑</a>
                            <a href="<?php echo U('setStatus?ids='.$distribution['id'].'&status='.abs(1-$distribution['status']));?>" class="ajax-get"><?php echo (show_status_op($distribution["status"])); ?></a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$distribution['id']);?>">删除</a>
                        </td>
