...
<!-- 主体 -->
    ...
    <ul class="tab pinjia order_mana">
        <li <?php if(($state_type) == "all"): ?>class="active"<?php endif; ?>><a href="<?php echo U('Refund/index');?>">所有（<?php echo ($ordersum); ?>）</a></li>
        <li <?php if(($state_type) == "state_new"): ?>class="active"<?php endif; ?>><a  href="<?php echo U('Refund/index',array('state_type'=>'state_new'));?>">等待处理（<?php echo ($noevalnum); ?>）</a></li>
        <li <?php if(($state_type) == "state_success"): ?>class="active"<?php endif; ?>><a  href="<?php echo U('Refund/index',array('state_type'=>'state_success'));?>">已完成（<?php echo ($evalnum); ?>）</a></li>
    </ul>
    <?php if(!empty($orderlist)): ?><div id='conten_id'>
            <?php if(is_array($orderlist)): $i = 0; $__LIST__ = $orderlist;if( count($__LIST__)==0 ) : echo "没有订单" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><div class="order_list">
                    <div class="two_clum">
                        <span class="fl"><i>订单号:</i><a href="<?php echo ($order["detail_url"]); ?>"><?php echo ($order["refund_sn"]); ?></a></span>
                        <!--<span class="fr"><?php echo ($order["orderStatus"]["status_txt"]); ?></span>--> 
                        <?php if(($order["refund_type"]) == "1"): ?><span class="fr" style="background-color: yellowgreen; color: #fff; line-height: 0.5rem; padding: -1px 2px">退款订单</span><?php endif; ?>
                        <?php if(($order["refund_type"]) == "2"): ?><span class="fr" style="background-color: green; color: #fff; line-height: 0.5rem; padding: -1px 2px;">退货订单</span><?php endif; ?>
                    </div>
                    <ul>
                    ...
                    <div class="two_clum">
                        <span class="fl"><i>时间：</i><?php echo (date("Y-m-d H:i:s",$order["add_time"])); ?></span>
                        <span class="fr">总额：<span class="cost">￥<?php echo ($order["refund_amount"]); ?></i></span> 
                    </div>
                    <div class="buy_agin">
                        <a href="<?php echo U('Refund/detail',array('id'=>$order['refund_id']));?>" class="pj">查看详情</a>
                    </div>
