...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    elseif (C('URL_MODEL') == 2) { //开启路由和Rewrite
        ...
        if (isset($router_ruler[$router_key])) { //路由命中
            ...
            if (!empty($vars)) { // 添加参数 
                ...
                foreach ($vars as $var => $val) {
                    ...
                    }else if ( strpos($url, '[' . $var . '|0]') ){
                        if ($val !== '') {
                            $url = str_replace('[' . $var . '|0]', $val, $url);
                        }
                    }else {
                        if ('' !== trim($val)) {
                            $other_vars_ymk[$var]=$val;
                            $other_vars .= $depr . $var . $depr . urlencode($val);
                        } 
                    }
                } 
                $url = str_replace('[domainid|0]', 0, $url);
                $url = str_replace('[brand|0]', 0, $url);
                $url = str_replace('[order|0]', 1, $url);
                $url = str_replace('[sort|0]', 1, $url);
                $url = str_replace('[type|0]', 0, $url);
                $url = str_replace('[p|0]', 1, $url);
                $url = str_replace('[id|0]',0, $url);
                ...
                if ($other_vars != '') { 
                    ...
                    $pars = http_build_query($other_vars_ymk); 
                    $url.="?".$pars;
                    $suffix=false;   
                }
                if ($suffix) { 
            ...
            //$url = preg_replace('/p\[p\].html/', '1', $url); 
            ...
            if (isset($route)) {
                ...
                $url = __APP__ . '/index.php?s=' . ($module ? $module . MODULE_PATHINFO_DEPR : '') . implode($depr, array_reverse($var));
            }
            if ($urlCase) {
                $url = strtolower($url);
            }
        ...
        $url = str_replace('zt/lists_1.html','zt/', $url);
        $url = str_replace('article/lists_0_1.html','article/', $url);
        $url = str_replace('product/lists_0_0_0_1_1_1.html','product/', $url);
        $url = str_replace('comment/lists_0_0_1.html','comment/', $url);
    }
