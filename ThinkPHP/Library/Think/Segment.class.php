...
class Segment {
	...
	function segment($str){
		//require_once (dirname(__FILE__) . '/Segment/pscws4.class.php');
		$class= 'Think\\Segment\\PSCWS4';
		$cws = new $class();
		$cws->set_charset('utf8');
		$cws->set_dict(dirname(__FILE__) . '/Segment/etc/dict.utf8.xdb');
		$cws->set_rule(dirname(__FILE__) . '/Segment/etc/rules.utf8.ini');
		$cws->set_ignore(true);
		$cws->set_multi(1);
		...
		while ($some = $cws->get_result())
		{
			...
			{
				$target_py .= ' '.$this->convert_to_py(strtolower($word['word']));
				$target_cn .= ' '.strtolower($word['word']);
			}
		}
		$cws->close();
		return array('py'=>$target_py,'cn'=>$target_cn);
	}
